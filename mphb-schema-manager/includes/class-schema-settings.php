<?php
/**
 * Admin settings: the two non-document layers.
 *  - Site defaults    (option Schema::OPT_SITE)
 *  - Cottage defaults (option Schema::OPT_COTTAGE) — the accommodation-type
 *    template default every cottage inherits.
 * Owns the top-level "MPHB Schema" menu and the Health screen.
 */
...
{
    public const GROUP_SITE    = 'mphbsch_schema_site';
    public const GROUP_COTTAGE = 'mphbsch_schema_cottage';
    public const HEALTH_SLUG   = 'mphbsch-schema-health';

    public static function register_menu(): void
    {
        add_menu_page(
            __('MPHB Schema', 'mphb-schema-manager'),
            __('MPHB Schema', 'mphb-schema-manager'),
            'manage_options',
        ...
        add_submenu_page(
            Schema_Importer::PARENT_SLUG,
            __('Defaults', 'mphb-schema-manager'),
            __('Defaults', 'mphb-schema-manager'),
            'manage_options',
            Schema_Importer::PARENT_SLUG,
            [self::class, 'render_settings']
    ...
    public static function render_settings(): void
    {
        ...
        echo '<div class="wrap"><h1>' . esc_html__('MPHB Schema — Defaults', 'mphb-schema-manager') . '</h1>';
        echo '<h2 class="nav-tab-wrapper">';
        printf('<a href="%s" class="nav-tab %s">%s</a>', esc_url(add_query_arg('tab', 'site', $base)), $tab === 'site' ? 'nav-tab-active' : '', esc_html__('Site-wide', 'mphb-schema-manager'));
        printf('<a href="%s" class="nav-tab %s">%s</a>', esc_url(add_query_arg('tab', 'cottage', $base)), $tab === 'cottage' ? 'nav-tab-active' : '', esc_html__('Cottage defaults', 'mphb-schema-manager'));
        echo '</h2>';

        if ($tab === 'cottage') {
            echo '<p>' . esc_html__('Configure the schema every accommodation type inherits. Use tokens like {{cottage_name}}, {{mphb_price}}, {{mphb_availability}} — they fill in per cottage. Override individual cottages in their Elementor Settings tab.', 'mphb-schema-manager') . '</p>';
            $group  = self::GROUP_COTTAGE;
            $option = Schema::OPT_COTTAGE;
            $scope  = Schema_Types::SCOPE_COTTAGE;
        } else {
            echo '<p>' . esc_html__('Site-wide identity emitted on every page and used as the parent entity the page graph links to.', 'mphb-schema-manager') . '</p>';
    ...
    private static function render_type_table(string $option, string $key, array $def, array $values): void
    {
        ...
        echo '<tr><th scope="row">' . esc_html__('Enable', 'mphb-schema-manager') . '</th><td>';
        printf(
            '<label><input type="checkbox" name="%s[%s][enable]" value="yes" %s> %s</label>',
            esc_attr($option),
            esc_attr($key),
            checked($enable, true, false),
            esc_html__('Emit this type', 'mphb-schema-manager')
        );
        echo '</td></tr>';
        ...
        foreach ((array) ($def['fields'] ?? []) as $field) {
            ...
            switch ($field['type'] ?? 'text') {
                case 'textarea':
                    ...
                    break;
                case 'switcher':
                    printf('<label><input type="checkbox" name="%s" value="yes" %s></label>', $attr, checked($val === 'yes', true, false));
                    break;
                case 'number':
    ...
    public static function render_health(): void
    {
        ...
        echo '<div class="wrap"><h1>' . esc_html__('Schema Health', 'mphb-schema-manager') . '</h1>';
        echo '<p>' . esc_html__('Detected JSON-LD on key pages (parsed from the live HTML — the same markup the external validators see). Duplicate types across blocks can cause validation errors.', 'mphb-schema-manager') . '</p>';

        $urls = ['home' => home_url('/')];
        foreach (array_slice(Data::list_room_types(), 0, 8) as $rt) {
        ...
        echo '<table class="widefat striped"><thead><tr>'
            ...
            . '<th>' . esc_html__('Blocks', 'mphb-schema-manager') . '</th>'
            . '<th>' . esc_html__('Invalid', 'mphb-schema-manager') . '</th>'
            . '<th>' . esc_html__('Types detected', 'mphb-schema-manager') . '</th>'
            . '<th>' . esc_html__('Test', 'mphb-schema-manager') . '</th>'
            . '</tr></thead><tbody>';
        foreach ($urls as $label => $url) {
            ...
            echo '<td>' . esc_html($types) . ($dupes ? ' <strong style="color:#b32d2e">(' . esc_html__('duplicate types', 'mphb-schema-manager') . ')</strong>' : '') . '</td>';
            echo '<td><a href="' . esc_url(Schema_Validator::validator_url((string) $url)) . '" target="_blank" rel="noopener">' . esc_html__('Validate', 'mphb-schema-manager') . '</a></td>';
            echo '</tr>';
        }
