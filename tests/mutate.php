...
$mutations = [
    ...
    // --- 0.42.0: the CSS build step and the deferred cottage panels ---
    ["build: the enqueue goes back to the commented source", 'class-widget.php',
     "MPHBAC_URL . 'assets/css/' . self::stylesheet_file(),",
     "MPHBAC_URL . 'assets/css/widget.css',",
     'build-test.php'],
    ["build: SCRIPT_DEBUG stops serving the commented source", 'class-widget.php',
     "return (defined('SCRIPT_DEBUG') && SCRIPT_DEBUG) ? 'widget.css' : 'widget.min.css';",
     "return 'widget.min.css';",
     'build-test.php'],
    ["build: data-no-minify is dropped now that comments are stripped at build time", 'class-widget.php',
     "'<link data-no-optimize=\"1\" data-no-minify=\"1\" '",
     "'<link data-no-optimize=\"1\" '",
     'build-test.php'],
    ["build: the committed .min.css goes stale against its source", 'assets/css/widget.min.css',
     ".mphbac-root,\n.mphbac-sheet,\n.mphbac-info-sheet {",
     ".mphbac-root,\n.mphbac-sheet {",
     'build-test.php'],
    ["lazy: the fragment stops carrying the template CSS, as v0.6.0 did", 'class-widget.php',
    ...
    ["lazy: the panel is re-fetched on every open instead of being kept", 'assets/js/widget.js',
     ...
     "            void content;",
     'lazy-panel-test.php'],
    ["lazy: a failed fetch is remembered, so the cottage can never be opened again", 'assets/js/widget.js',
