...
(function () {
	...
	function initRoot(root) {
		...
		/* ---------- sightings bar + drawer ---------- */

		function renderRecent(section, items) {
			var list = section.querySelector('.dccwl-sightings-list');
			if (!list) {
				return;
			}
			list.textContent = '';
		...
		function updateBar(section, items) {
			var text = section.querySelector('.dccwl-sightbar-text');
			var cta = section.querySelector('.dccwl-sightbar-cta');
			if (!text || !cta) {
				return;
			}
			if (items.length && speciesById[items[0].species]) {
				var s = speciesById[items[0].species];
				text.textContent = CFG.i18n.latest + ' ' + s.emoji + ' ' + s.name + ' · ' + String(items[0].date || '');
				cta.textContent = CFG.i18n.logYours;
			} else {
				text.textContent = CFG.i18n.spotted;
				cta.textContent = CFG.i18n.logIt;
			}
		}
		...
		function loadRecent(section) {
			post('dcc_wl_recent', {}).then(function (res) {
				if (res && res.success && res.data && Array.isArray(res.data.items)) {
					renderRecent(section, res.data.items);
					updateBar(section, res.data.items);
				}
			}).catch(function () { /* leave the bar/list as-is */ });
		}
		...
		function buildForm(section, slot) {
			...
			form.appendChild(field(CFG.i18n.species, select, fuid + '-species'));
			...
			date.max = todayLocal();
			form.appendChild(field(CFG.i18n.date, date, fuid + '-date'));
			...
			form.appendChild(field(CFG.i18n.note, note, fuid + '-note'));
			...
			form.appendChild(field(CFG.i18n.firstName, name, fuid + '-name'));
		...
		function initSightings() {
			...
			var bar = section.querySelector('.dccwl-sightbar');
			var drawerSlot = section.querySelector('.dccwl-drawer-slot');
			var drawer = section.querySelector('.dccwl-drawer');
			if (drawer) {
				drawer.id = uid + '-drawer';
			}
			if (bar && drawerSlot) {
				bar.setAttribute('aria-controls', uid + '-drawer');
				bar.addEventListener('click', function () {
					var open = drawerSlot.classList.toggle('dccwl-open');
			...
			if (btn && slot) {
				btn.addEventListener('click', function () {
					...
					slot.textContent = '';
					var form = buildForm(section, slot);
