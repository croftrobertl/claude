...
- **DCC Wildlife** (`dcc-wildlife/`) — "On the Canal This Month" wildlife spotlight/field guide, plus a source-gated fishing & water-conditions module. Release zips are named **`Wildlife <version>.zip`** (e.g. `Wildlife 1.23.0.zip`). Everything ELSE handed to the owner — images, markdown, JS — is copied to `Wildlife - <Title>.<ext>` first; the zips are deliberately exempt from that prefix — see `dcc-wildlife/CLAUDE.md` for the build command that derives the version from the plugin header. Has its own `dcc-wildlife/CLAUDE.md` — read it before touching that plugin (different invariants: 100% WordPress-native, client-side month selection, zero AJAX; the guest sightings log was removed in v1.2.0 — restore from git history at v1.1.0 if ever wanted; since v1.8.0 it also owns the season countdown, so the mu-plugin `dcc-wildlife-countdown.php` is deletable once that is verified live; v1.9.0 rebuilt its UI in the DCC Guest Guide's visual language — tiles, a shared sliding sheet, a token layer in `assets/css/app.css` — v1.9.1 swapped in the Guide's measured palette and made the widget ALWAYS LIGHT like the Guide, so never add an OS dark-mode rule to it; v1.10.0 restructured the front end into one hub app — the `dccwl_canal` widget / `[dcc_canal]` — which COMPOSES the older renderers rather than forking them, so the legacy widgets still work standalone; v1.19.0 began Phase 2 — the guide grows towards 169 species on a group/flags/odds/place model with photo-first tiles, one reviewed batch at a time — see `dcc-wildlife/CLAUDE.md`).
...
- **The site-wide Elementor category for DCC widgets is `dcc-widgets`** ("Dora Canal Court") — all DCC plugins (Guest Guide, Contact Form, Availability Calendar, Features & Amenities, DCC Wildlife) share it, each registering it idempotently. Earlier revisions of this doc said `claude-code`; that slug is stale — use `dcc-widgets` for any new or updated widget. (This repo's mphb-availability-calendar code still registers `claude-code` in `Plugin::register_category()`; migrate it to `dcc-widgets` next time that plugin is touched. Elementor stores placed widgets by widget type, not category, so the switch doesn't affect existing pages.)
