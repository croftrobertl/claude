...
- **The Elementor widget category is `dcc-widgets`** ("Dora Canal Court") — the slug every live DCC plugin registers (Guest Guide, Contact Form, Features & Amenities), and now both plugins in this repo: Cottage Selector since 0.21.1, Availability Calendar since 0.9.9. Never invent a different slug: Elementor groups the panel by slug, so a mismatch creates a duplicate "Dora Canal Court" section (the 0.17.1 incident). The only remaining `claude-code` mentions in the repo are historical — a warning comment in each plugin and past changelog entries recording what those releases actually did; they are deliberately left as-is.
