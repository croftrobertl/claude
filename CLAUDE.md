...
**The pipeline is two steps, and each arrow has its own `--check`:**

```
score/labels/availability/selector  --build-bundle.php-->  dccs.js  --build-min.js-->  dccs.min.js
```

`dccs.min.js` is what the front end loads; `dccs.js` is what `SCRIPT_DEBUG`
serves, the same arrangement as the stylesheet. **The sources are the source of
truth; both built files are generated and must never be hand-edited.** `npm test`
runs both check modes and fails if either is stale, because a stale artefact at
either step means the repo and the site disagree while every other test passes.

**The minify step is comments and whitespace ONLY — no compress, no mangle.**
Owner's decision (0.46.0): mangling buys ~3.3 KB more and costs the ability to
read a stack trace from a guest's browser, on a widget that gets debugged on the
live site. A test asserts specific identifiers survive verbatim, so turning
mangling on fails loudly rather than quietly. Measured at the host's ~1 KB window:
41.2 KB → 19.5 KB, 22.2 KB saved per page.

`dccs.js` remains a PLAIN CONCATENATION, which is why the older checks that assert
it contains each source verbatim and in dependency order are still valid — only
the shipped file is transformed. Keeping the two artefacts separate preserved that
check instead of trading it away.

**Until 0.46.0 nothing in either suite ever EXECUTED a built artefact.** Every one
of the 600-odd assertions boots the four sources, so a broken bundle would have
shipped green. The suite now also renders five scenarios twice — once from the
sources, once from `dccs.min.js` alone — and compares the markup, with a positive
control proving the comparator can report a difference. `?seed=` pins the
tie-break rotation so both renders agree regardless of the clock.

The old per-file handles (`dccs-score` and friends) were removed rather
